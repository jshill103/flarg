...
namespace test
{
    ...
    public partial class Form1 : Form
    {
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            ...
            market = new Bitmap(Properties.Resources.market);
            sellTab = new Bitmap(Properties.Resources.sellTab);
            bagButton = new Bitmap(Properties.Resources.bagButton);
            claim = new Bitmap(Properties.Resources.claim);
            empty = new Bitmap(Properties.Resources.empty);
            itemTab = new Bitmap(Properties.Resources.itemTab);
            item = new Bitmap(Properties.Resources.item);
            pricePerUnit = new Bitmap(Properties.Resources.priceInput);
        ...
        private async Task listItem()
        {
                ...
                //Find price per unit img and click under it
        ...
        private List<ScanCodeShort> findScanCodes(string input)
        {
            List<ScanCodeShort> values = new List<ScanCodeShort>();
            for (var i = 0; i < input.Length; i++)
            {
                values.Insert(i, matchInputToScanCode(input[i]));
            }
            return values;
        }

        private ScanCodeShort matchInputToScanCode(char c)
        {
            ScanCodeShort val = new ScanCodeShort();
            
            var input = Char.GetNumericValue(c);
            switch ((int)input)
            {
                case 1:
                    val = (ScanCodeShort)2;
                ...
                case 8:
                    ...
                    break;
                case 9:
                    val = (ScanCodeShort)10;
                    break;
                case 0:
                    val = (ScanCodeShort)11;
                    break;
            }
            return val;
        }

        private void sendKeyInput(ScanCodeShort value)
        {
        ...
        private void sendKeyInputUp(ScanCodeShort value)
        {
            INPUT[] Inputs = new INPUT[1];
            INPUT Input = new INPUT();
        ...
        private Point? checkScreen(Bitmap image)
        {
            SetForegroundWindow(windowHandle);
            Bitmap currScreen = PrintWindow(windowHandle);
        ...
        public void LeftClick(Point point)
        {
            ...
            mouse_event((int)(MouseEventFlags.LEFTUP), 0, 0, 0, 0);
            Cursor.Position = ConvertToScreenPixel(point);
            mouse_event((int)(MouseEventFlags.LEFTDOWN), 0, 0, 0, 0);
            mouse_event((int)(MouseEventFlags.LEFTUP), 0, 0, 0, 0);
        ...
        public void LeftClick(Point point, int xOffset, int yOffset)
        {
            Point tmp = Cursor.Position;
        ...
        private Point ConvertToScreenPixel(Point point)
        {
            RECT rect;
